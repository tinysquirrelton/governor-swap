...
class Swap extends Component {
  constructor(props) {
	...
	this.swapContractABI = [{"inputs":[{"internalType":"address","name":"_synthetico","type":"address"},{"internalType":"address","name":"_authentico","type":"address"},{"internalType":"uint256","name":"_inicio","type":"uint256"}],"stateMutability":"nonpayable","type":"constructor"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"_purchaser","type":"address"},{"indexed":true,"internalType":"uint256","name":"_tokens","type":"uint256"}],"name":"purchased","type":"event"},{"inputs":[],"name":"authentico","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"inicio","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},{"inputs":[{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"purchase","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"synthetico","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"}];
  ...
  connectMainnet = async (accounts) => {
    await this.web3?.eth?.getChainId().then((x) => {
      if (x == 1) {
	    ...
	    this.SLPContract = new this.web3.eth.Contract(this.SLPABI, this.SLPAddress);
	    this.LPContract = new this.web3.eth.Contract(this.LPABI, this.LPAddress);
	    this.swapContract = new this.web3.eth.Contract(this.swapContractABI, this.swapContractAddress);
  ...
  getSwapStats = () => {
	...
	if(this.SLPContract != null && this.LPContract != null) {
	  this.SLPContract.methods.balanceOf(this.state.account).call().then(result => {
	    this.setState({ userSLP: parseFloat(this.web3.utils.fromWei(result, 'ether')), userSLPRaw: result });
	  });
	  this.LPContract.methods.balanceOf(this.swapContractAddress).call().then(result => {
	    this.setState({ unclaimed: parseFloat(this.web3.utils.fromWei(result, 'ether')) });
	  });
	  this.SLPContract.methods.balanceOf(this.swapContractAddress).call().then(result => {
	    this.setState({ claimed: parseFloat(this.web3.utils.fromWei(result, 'ether')) });
	  });
	  this.SLPContract.methods.allowance(this.state.account, this.swapContractAddress).call().then(result => {
  ...
  approveContract = () => {
	if(this.web3 != null && this.SLPContract != null) {
      this.SLPContract.methods.approve(this.swapContractAddress, this.state.userSLPRaw).send({
        ...
        toast.success("You have approved the swap contract to spend your sLP");
		this.getSwapStats();
	  });
	}
  }
  ...
  swapTokens = () => {
	if(this.web3 != null && this.swapContract != null) {
      this.swapContract.methods.purchase(this.state.userSLPRaw).send({
	    from: this.state.account
	  }).on('error', function (error) {
        toast.error("Transaction was not successful");
	  }).on('transactionHash', function (transactionHash) {
  ...
  componentDidMount() {
	...
	if(startCountdown > now) {
	  let countdownInterval = setInterval(function() {
		...
		let calculatedCountdownString = days + ":" + hours + ":" + minutes + ":" + seconds;
      ...
      <div className="max-width-container">
        <div className="airdrop-container">
          ...
          <div className="airdrop-subtitle">
		    <a href="https://etherscan.io/address/0xcc23ef76b46ed576caa5a1481f4400d2543f8006#code" target="_blank" style={{fontSize: '0.8em', color: '#ffffff', display: 'inline-block', textAlign: 'center'}}>Swap Contract</a>
          </div>
			    ...
			    this.state.isConnected ? (
				  <>
					<div className="claim-item">
					  <div className="title">You're about to claim {this.state.userSLP.toLocaleString()} LP</div>
					</div>
					{this.state.userSLP >= this.state.SLPAllowance ? (
					  <button className="claim-btn" onClick={this.approveContract}>Approve</button>
					) : (
					  <button className="claim-btn" onClick={this.swapTokens}>Swap tokens</button>
					)}
				  </>
                ) : (
